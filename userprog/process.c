...
/*******************************************************************
 * 执行此函数时，当前任务可能是线程
 * 之所以对线程页要重新安装页表，原因是上一次被调度的可能是进程，
 ...
 *******************************************************************/
    ...
    //注意内核线程的pcb中pgdir等于NULL,
    //而线程的pcb中pgidr为指向页表的虚拟地址
...
/* 激活线程或进程的页表，更新tss中的esp0为进程大的特权级0的栈 */
/* 只有在任务调度时才会切换页表及更新0级栈，因此
 * process_activate是被schedule调用的 */
void process_activate(struct task_struct* p_thread) {
    ASSERT(p_thread != NULL);
    /* 激活该进程或线程的页表 */
    page_dir_activate(p_thread);

    /* 内核线程特权级本身就是0,处理器进入中断时
     * 并不会从tss中获取0特权级栈地址，故不需要更新esp0 */
    if (p_thread->pgdir) {
...
uint32_t* create_page_dir(void) {
    ...
    /**************先复制页表***************************************/
    /* page_dir_vaddr + 0x300*4 是新页目录表的第768项 */
...
void create_user_vaddr_bitmap(struct task_struct* user_prog) {
    ...
    user_prog->userprog_vaddr.vaddr_bitmap.bits =\
            get_kernel_pages(bitmap_pg_cnt); //内核空间分配
