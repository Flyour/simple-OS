...
/* 定义以下变量记录相应键是否按下的状态，
 * ext_scancode用于记录makecode是否一0xe0开头 */
static _Bool ctrl_status, shift_status, alt_status, caps_lock_status, ext_scancode;
...
static void intr_keyboard_handler(void) {
    ...
    /*若为通码，只处理数组中定义的键以及alt_right 和ctrl键，
     * 全是make_code */
        ...
        if ((scancode < 0x0e) || (scancode == 0x29) || \
            ...
            if (shift_down_last && caps_lock_last) {
                shift = false;  //如果shift和capslock同时按下
            } else if (shift_down_last || caps_lock_last) {
                shift = true;   //如果shift和capslock任意被按下
            } else {
                shift = false;
            }
        }
