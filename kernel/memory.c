...
/********************位图地址******************************************
 * 因为0xc009f000是内核主线程栈顶，0xc009e000是内和主线程的pcb
 * 一个叶框大小的位图可表示128MB内存，位图位置安排在地址0xc009a000，
 * 这样本系统最大支持4个也框的位图，即512MB */
...
struct pool {
    struct bitmap pool_bitmap;  //本内存用到的位图结构，用于管理物理内存
...
void* malloc_page(enum pool_flags pf, uint32_t pg_cnt){
     ...
     * 3、通过page_table_add将以上得到虚拟地址和物理地址在页表中完成映射
     ***************************************************************/
    ...
    /*U 因为虚拟地址是连续的，单物理地址可以是不连续的，所以逐个做映射*/
    while (cnt-- > 0) {
...
static void mem_pool_init(uint32_t all_mem){
    ...
    //0x100000 为低端1MB内存,used_mem表示已经被使用的物理内存

    uint32_t free_mem = all_mem - used_mem;
    uint16_t all_free_pages = free_mem / PG_SIZE ;
    //一页为4KB，不管总内存是不是4K的倍数

    uint16_t kernel_free_pages = all_free_pages / 2;
    uint16_t user_free_pages = all_free_pages - kernel_free_pages;

    /* 为简化位图操作，余数不处理，坏处是这样做会丢内存,
     * 好处是不用做内存的越界检查，因为位图表示的内存少与实际物理内存*/
...
void* sys_malloc(uint32_t size) {
    ...
    if (size > 1024) {
        ...
        if (a != NULL) {
            memset(a, 0, page_cnt * PG_SIZE);   //将分配大的内存清0
        ...
        /* 若为mem_block_desc的free_list中已经没有可用的mem_block,
...
void mem_init(){
    ...
    /* 初始化mem_blockk_desc数组descs,为mallock做准备*/
