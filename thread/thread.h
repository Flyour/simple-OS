...
/* 自定义通用函数类型，它将在很多线程函数中作为型参类型*/
typedef void thread_func(void*);
typedef int16_t pid_b;
 ...
 *************************************************************/
struct intr_stack {
    uint32_t vec_no;    //kernel.S宏VECTOR中push %1 压入的中断号
    uint32_t edi;
    uint32_t esi;
    uint32_t ebp;
    uint32_t esp_dummy;
...
struct task_struct {
    ...
    uint32_t* pgdir;    //用于存放进程页目录表的虚拟地址
    struct virtual_addr userprog_vaddr; //用户进程的虚拟地址池
    struct mem_block_desc u_block_desc[DESC_CNT];  //用户进程内存块描述符
    uint32_t stack_magic;   //一个魔数，用作栈的边界标记，检测栈的溢出
};

struct task_struct* thread_start(char* anme, \
        int prio, \
